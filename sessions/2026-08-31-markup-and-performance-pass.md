...
A third one reached Pablo's screen before it was caught. The `width` and `height` attributes added in step 3 act as presentational hints, so `height="739"` resolved to a used height; together with the author's `width: 100%` that specifies both dimensions, and a fully specified box makes `aspect-ratio` inert. `.article-card img` lost its 16/9 crop and rendered the whole 1200x739 source. `.content-view > img` got `height: auto` during step 3 for exactly this reason and `.article-card img` did not, because its `aspect-ratio` declaration made it look already handled. Fixed by adding `height: auto` there too; verified at 558x314, ratio 1.78, on all five cards. The lesson generalises: adding dimension attributes to an image requires auditing every author rule that sets one dimension, not only the ones that set none.
