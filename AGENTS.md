...
`portfolio` is a surface, not a source: `../my-career/` holds the content and decides the work, the way `../arg-work/` does for `../mi-argentina-front/`. So this repo carries no `PENDING.md`, no `BACKLOG.md`, no `HANDOFF.md` and no `QUEUE.md`. Anything pending about these pages is an item in `../my-career/PENDING.md`, tagged `(portfolio)`.
...
Articles come from `../my-career/articles/` drafts. The repo was `blog-articles` until it was renamed on 2026-07-16, and `article-guidelines.md` did not survive the rename; what governed articles is now split in two:

- **`../my-career/AGENTS.md`** § *Publication format* — opening structure, image types that work, video behavior (autoplay loop muted playsinline), what to avoid
